import IntegerFormInput from 'formInput/IntegerFormInput'

describe('IntegerFormInput', () => {
    it('generates new value correctly', () => {
        const input = new IntegerFormInput()
        ...
        assert.equal(1, input.getNewValue(null, {payload: {value: 1}}))
        assert.equal(2, input.getNewValue(null, {payload: {value: '2'}}))
        assert.equal(3, input.getNewValue(null, {payload: {value: 3.3}}))
    })
    it('preforms basic validation correctly', () => {
        const input = new IntegerFormInput()
        assert.notEqual(null, input.validateBasic({}))
        assert.notEqual(null, input.validateBasic(123.3))
